...
export default function Register({ documents }: RegisterPropsType) {
	...
	const mappedDocuments = documents.map((doc) => ({
		...
		file: null, // or '' — initialize as empty, can later hold an uploaded file
	}));
	...
	const handleFileChange = (index: number, file: File | null) => {
		setData(prev => {
			...
			while (files.length < uploadLabels.length) files.push(null);
			files[index] = file;
			return { ...prev, files };
		});
	};
					...
					<div className="flex flex-col gap-5">
						...
						<div className="flex flex-col gap-5 bg-gray-100/50 rounded-md p-5 py-10 border border-gray-200">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
								...
								<RegisterImageUpload onImageChange={handleImageChange} />
									...
									<div className="bg-gray-50 p-4 rounded-lg border">
										<div className="flex items-center gap-3">
											<div className="w-16 h-16 rounded-full overflow-hidden bg-white">
												<img
													src={URL.createObjectURL(personalImage)}
													alt="الصورة الشخصية"
													className="w-full h-full object-cover"
												/>
											</div>
											<div>
												<p className="font-medium text-gray-800">{personalImage.name}</p>
												<p className="text-sm text-gray-500">{(personalImage.size / 1024).toFixed(1)} KB</p>
											</div>
										</div>
									</div>
								</div>
							...
							<div>
								...
								<div className="space-y-3">
									{data.files.map((file, index) => (
										<div key={index}>
											<PdfPreviewThumbnail file={file} label={uploadLabels[index]} />
										</div>
									))}
								</div>
							</div>
